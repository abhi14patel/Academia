...
public class ResultReportMethodAdapter extends RecyclerView.Adapter<ResultReportMethodAdapter.MyViewHolder>
{
    Context context;
    ArrayList<ResultReportMethod_Dto> resultReportMethodList;

    TranslationManager translationManager;
    LinearLayoutManager linearLayoutManager;
    private ArrayList<ResultReportEvent_Dto> resultReportEventList;
    ResultReportEventAdapter resultReportEventAdapter;

    public ResultReportMethodAdapter(Context context, ArrayList<ResultReportMethod_Dto> resultReportMethodList) {
        this.context = context;
        this.resultReportMethodList = resultReportMethodList;
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, int position) {
        ...
        ResultReportMethod_Dto list = resultReportMethodList.get(position);
        ...
        holder.ivArrow.setImageResource(R.drawable.arrow_green);

        resultReportEventList = new ArrayList<>();
        resultReportEventList = list.getEventList();
        if(resultReportEventList != null && resultReportEventList.size() > 0){

            holder.ivHeaderIndicator.setVisibility(View.VISIBLE);
            resultReportEventAdapter = new ResultReportEventAdapter(context,resultReportEventList);
            holder.recyclerView.setAdapter(resultReportEventAdapter);
    ...
    public int getItemCount() {
        ...
        return (null != resultReportMethodList ? resultReportMethodList.size() : 0);
    }
